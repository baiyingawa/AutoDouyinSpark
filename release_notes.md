...
### 🔧 优化
- 更新策略改为**优先加速源**：先走 douyin.uuovo.xyz 代理检测和下载，失败才回退 GitHub 直连
...
### 🐛 修复
- 自动更新在 GitHub 被墙地区无法获取版本信息和下载文件的问题，新增 douyin.uuovo.xyz 加速源作为备用 API 和下载源
- 下载页面路径写死导致页面无法正常显示的问题

### ✨ 新增
- 自动更新检测：GitHub API 不可用时自动降级到 douyin.uuovo.xyz 代理
- 下载页进度条：XHR 流式传输带实时百分比和文件大小显示
- douyin.uuovo.xyz 加速下载服务：支持 SHA256 内容寻址缓存中转
...
### ✨ 新增
- 新增“状态”页，显示应用调度器和 Windows 计划任务状态。
- 多好友最多 8 线程并行续火，单个好友使用独立浏览器会话。
- 点击聊天头像卡片打开个人页后自动读取抖音号。

### 🐛 修复
- 使用跨进程原子锁，避免计划任务和手动任务重复启动、重复发送。
- 搜索结果改为精确好友匹配，识别末尾带人数括号的群聊并跳过。
- 截图改为保存到统一数据目录，并使用临时文件原子替换，避免截图错位或半截文件。
- 修复截图列表无法识别同名截图被更新的问题。
- 登录 Cookie 保存与首页状态判断流程稳定性优化。

### 📦 发布
- Windows x64 安装包：`AutoDouyinSpark Setup 2.8.0.exe`
## v2.9.0 (2026-09-03)
